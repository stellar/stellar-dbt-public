...
with
    ...
    , transformed_data as (
        select
            ...
            , cc.batch_id
            , cc.batch_run_date
        from {{ ref('int_incr_contract_code') }} as cc
        left join derived_data as dd on cc.contract_code_hash = dd.contract_code_hash
    )
    ...
    , dedup_data as (
        select
            *
            ...
            , lag(row_hash) over (partition by contract_code_hash order by closed_at, ledger_sequence)
                as prev_row_hash
