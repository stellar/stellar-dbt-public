...
with
    base_trades as (
        ...
        where
            ...
            and ledger_closed_at >= timestamp(date('{{ dbt_airflow_macros.ds() }}'))
    )
    ...
    , trade_day_agg_group as (
        select
            ...
            , sum(asset_b_amount) / sum(asset_a_amount) as avg_price_daily
            , max(price_n / price_d) as high_price_daily
            , min(price_n / price_d) as low_price_daily
    ...
    , join_table_daily as (
        select
            ...
            , trade_day_agg_group.avg_price_daily
            , trade_day_agg_group.high_price_daily
            , trade_day_agg_group.low_price_daily
            , trade_day_agg_window.open_n_daily
            , trade_day_agg_window.open_d_daily
            , trade_day_agg_window.close_n_daily
            , trade_day_agg_window.close_d_daily
