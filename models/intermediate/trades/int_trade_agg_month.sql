...
with
    base_trades as (
        ...
        where
            ...
            and ledger_closed_at >= timestamp(date_sub(date('{{ var("batch_start_date") }}'), interval 31 day))
    )
    ...
    , trade_day_agg_group as (
        ...
        where ledger_closed_at >= timestamp(date_sub(date('{{ var("batch_start_date") }}'), interval 30 day))
        group by
            asset_a
            , asset_a_code
            , asset_a_issuer
