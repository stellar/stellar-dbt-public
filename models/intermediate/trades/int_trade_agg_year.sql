...
-- traded, emits the trailing 365-day (D-365 .. D inclusive) rolling metrics as of
...
with
    ...
    /* trailing 365-day rolling window (D-365 .. D inclusive) per asset pair */
    , rolling_yearly as (
        select
            day_agg
            , asset_a
            ...
            , asset_b_issuer
            , asset_b_type
            , sum(daily_trade_count) over w as trade_count_yearly
            , sum(daily_asset_a_volume) over w as asset_a_volume_yearly
            , sum(daily_asset_b_volume) over w as asset_b_volume_yearly
            , sum(daily_asset_b_volume) over w
                / sum(daily_asset_a_volume) over w as avg_price_yearly
            , max(daily_high_price) over w as high_price_yearly
            , min(daily_low_price) over w as low_price_yearly
            , first_value(day_open_n) over w as open_n_yearly
            , first_value(day_open_d) over w as open_d_yearly
            , day_close_n as close_n_yearly
            , day_close_d as close_d_yearly
        from daily_metrics
        window
            w as (
